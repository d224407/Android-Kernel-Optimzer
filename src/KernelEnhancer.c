 ...
 * Dịch chính xác từ pseudo‑C gốc, dọn dẹp rác dịch ngược và sửa lỗi Memory Overlap.
...
static void tune_block_device(const char *devpath) {
    ...
    if (stat(path, &st) == 0 && S_ISREG(st.st_mode)) write_file(path, "256");

    snprintf(path, sizeof(path), "%s/nr_requests", devpath);
    if (stat(path, &st) == 0 && S_ISREG(st.st_mode)) write_file(path, "64");

    snprintf(path, sizeof(path), "%s/rq_affinity", devpath);
    if (stat(path, &st) == 0 && S_ISREG(st.st_mode)) write_file(path, "2");

    snprintf(path, sizeof(path), "%s/iostats", devpath);
    if (stat(path, &st) == 0 && S_ISREG(st.st_mode)) write_file(path, "0");
...
int main(int argc, char **argv) {
    ...
    // ----- Xác định đường dẫn module (Đã fix lỗi Memory Overlap) -----
    if (argc > 0) {
        char temp_path[256];
        strncpy(temp_path, argv[0], sizeof(temp_path)-1);
    ...
    if (mem_kb > 0) {
        ...
        snprintf(val, sizeof(val), "%d", swappiness); write_file("/proc/sys/vm/swappiness", val);
        snprintf(val, sizeof(val), "%d", dirty_ratio); write_file("/proc/sys/vm/dirty_ratio", val);
        snprintf(val, sizeof(val), "%d", dirty_bg); write_file("/proc/sys/vm/dirty_background_ratio", val);
        ...
        snprintf(val, sizeof(val), "%d", vfs_pressure); write_file("/proc/sys/vm/vfs_cache_pressure", val);
        write_file("/proc/sys/vm/stat_interval", "21");
        snprintf(val, sizeof(val), "%d", watermark); write_file("/proc/sys/vm/watermark_scale_factor", val);
    ...
    if (stune_base) {
        snprintf(path, sizeof(path), "%s/top-app/schedtune.boost", stune_base); write_file(path, "3");
        snprintf(path, sizeof(path), "%s/top-app/schedtune.prefer_idle", stune_base); write_file(path, "0");
        snprintf(path, sizeof(path), "%s/foreground/schedtune.boost", stune_base); write_file(path, "0");
        snprintf(path, sizeof(path), "%s/background/schedtune.boost", stune_base); write_file(path, "-10");
            ...
            if (access(sched_path, F_OK) == 0) {
                if (file_contains(sched_path, "mq-deadline")) {
                    ...
                    if (stat(iosched, &st) == 0 && S_ISDIR(st.st_mode)) {
                        snprintf(buf, sizeof(buf), "%s/read_expire", iosched); write_file(buf, "50");
                        snprintf(buf, sizeof(buf), "%s/write_expire", iosched); write_file(buf, "150");
                        snprintf(buf, sizeof(buf), "%s/writes_starved", iosched); write_file(buf, "1");
                        snprintf(buf, sizeof(buf), "%s/front_merges", iosched); write_file(buf, "0");
                    }
                    ...
                    snprintf(iosched, sizeof(iosched), "%s/iosched", path);
                    if (stat(iosched, &st) == 0 && S_ISDIR(st.st_mode)) {
                        snprintf(buf, sizeof(buf), "%s/slice_idle", iosched); write_file(buf, "0");
                        snprintf(buf, sizeof(buf), "%s/low_latency", iosched); write_file(buf, "1");
                        snprintf(buf, sizeof(buf), "%s/quantum", iosched); write_file(buf, "8");
                        snprintf(buf, sizeof(buf), "%s/group_idle", iosched); write_file(buf, "0");
                        snprintf(buf, sizeof(buf), "%s/back_seek_penalty", iosched); write_file(buf, "1");
                        snprintf(buf, sizeof(buf), "%s/back_seek_max", iosched); write_file(buf, "1000000000");
                        snprintf(buf, sizeof(buf), "%s/slice_sync", iosched); write_file(buf, "85");
                        snprintf(buf, sizeof(buf), "%s/slice_async", iosched); write_file(buf, "85");
                        snprintf(buf, sizeof(buf), "%s/slice_async_rq", iosched); write_file(buf, "2");
                        snprintf(buf, sizeof(buf), "%s/slice_async_us", iosched); write_file(buf, "75000");
                        snprintf(buf, sizeof(buf), "%s/target_latency_us", iosched); write_file(buf, "20000");
                        snprintf(buf, sizeof(buf), "%s/fifo_expire_sync", iosched); write_file(buf, "100");
                        snprintf(buf, sizeof(buf), "%s/fifo_expire_async", iosched); write_file(buf, "250");
                    }
                }
            }
    ...
    if (dir) {
        while ((ent = readdir(dir)) != NULL) {
            if (strncmp(ent->d_name, "zram", 4) == 0) {
                ...
                if (access(path, F_OK) == 0) write_file(path, "32");
            }
        }
        closedir(dir);
    }
    ...
    if (dir) {
        while ((ent = readdir(dir)) != NULL) {
            ...
            if (stat(sdir, &st) != 0 || !S_ISDIR(st.st_mode)) strcpy(sdir, path);

            snprintf(buf, sizeof(buf), "%s/rate_limit_us", sdir); write_file(buf, "500");
            snprintf(buf, sizeof(buf), "%s/up_rate_limit_us", sdir); write_file(buf, "500");
            snprintf(buf, sizeof(buf), "%s/down_rate_limit_us", sdir); write_file(buf, "500");
            snprintf(buf, sizeof(buf), "%s/hispeed_load", sdir); write_file(buf, "90");
        }
        closedir(dir);
